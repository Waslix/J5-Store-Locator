 ...
 * @since  1.0.0
 */
class LocationfieldModel extends AdminModel
{
	/**
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_store_locator.locationfield';
	 ...
	 * @since   1.0.0
	 */
	public function getTable($type = 'Locationfield', $prefix = 'Administrator', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm(
								'com_store_locator.locationfield', 
								'locationfield',
								array(
									'control' => 'jform',
									'load_data' => $loadData 
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_store_locator.edit.locationfield.data', array());
	...
	public function save($data)
	{
		...
		$app = Factory::getApplication();
        $def = $app->input->get('jform', '', 'array');
		$fopt = $app->input->get('field_options', '', 'array');
		$defs = $def['default_value'];
		$plc = $def['placeholder'];
		$cls = $def['class'];
		$req = $def['required'];
		$nme = $def['name'];
		$nme = OutputFilter::stringURLSafe($nme);
		$lbl = $def['label'];
		$dtm = array(
			'default' => $defs,
			'placeholder' => $plc,
